...
namespace ProcessControlService.ResourceLibrary.Machines.DataSources
{
    ...
    /// FX 以太网
    /// </summary>
    public class MelsecFxNetDatasource : DataSource
    {
        private static readonly ILog LOG = LogManager.GetLogger(typeof(MelsecFxNetDatasource));
        private MelsecMcNet PLC;

        public string PlcAddress => PLC?.IpAddress;

        public MelsecFxNetDatasource(string name, Machine machine)
            : base(name, machine)
        {
        }
