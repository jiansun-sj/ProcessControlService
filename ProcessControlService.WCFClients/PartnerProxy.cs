...
namespace ProcessControlService.WCFClients
{
    /// <summary>
    /// 冗余伙伴检测服务代理，采用单工模式
    /// Created by Dongmin,2018/03/10
    /// </summary>
    public class PartnerProxy : ClientBase<IPartner>, IPartner, IProxyConnection,IDisposable
    {
        ...
        public PartnerProxy(System.ServiceModel.Channels.Binding binding, EndpointAddress edpAddr)
        ...
        public Int16 GetPartnerMode()
        {
            ...
            {
                if (base.State == CommunicationState.Opened)
                    return base.Channel.GetPartnerMode();
                else
                    return 0;
            }
            catch (Exception)
        ...
        //    {
        //        if (base.State == CommunicationState.Opened)
        //            base.Channel.TogglePartnerMode();
        //    }
        //    catch (Message)
        //    {
        //        LOG.Error("连接服务端出错，无法调用资源服务");
        ...
        public long GetPartnerRunTime()
        {
            ...
            {
                if (base.State == CommunicationState.Opened)
                    return base.Channel.GetPartnerRunTime();
                else
                    return 0;
            }
            catch (Exception)
        ...
        public void ExchangeData(string ResourceName, string ExchangeData)
        {
            ...
            {
                if (base.State == CommunicationState.Opened)
                     base.Channel.ExchangeData(ResourceName, ExchangeData);
            }
            catch (Exception)
            {
